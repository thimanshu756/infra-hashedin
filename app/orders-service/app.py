...
def create_order():
    ...
    if not data or not data.get('user_id') or not data.get('product_id'):
        return jsonify({"error": "Missing required fields: user_id, product_id"}), 400
    conn = get_db_connection()
    try:
        cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        cur.execute(
            "INSERT INTO orders (user_id, product_id, quantity) VALUES (%s, %s, %s) RETURNING *",
...
def update_order(order_id):
    ...
    try:
        cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        cur.execute(
            """UPDATE orders SET quantity = COALESCE(%s, quantity),
